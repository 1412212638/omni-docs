...
Skill 压缩包现在已经包含可执行的 Node 脚本，可以直接完成路由和提交：
...
如果你只想查看最终请求体，可以加 `--dry-run`；如果要查任务结果，可以用 `node scripts/get-task.mjs --task-id ... --family video-standard`。
...
- 公开的 `/v1/videos/generations` 示例使用的是单个 `image` URL
- 部分账户侧能力或素材重组场景可能会使用 `images` 数组，但同样要求公网 `http(s)` URL
